...
根據《調解條例》（第620章）第8條、PD 31 及 PD 31.1，調解通訊與和解會議內容具嚴格保密特權。
...
# --- 3. 知識庫初始化 (Cap 620 + PD31/31.1 + 司法機構 FAQ + 外部守則) ---
@st.cache_resource(show_spinner="正在加載《調解條例》、實務指示、司法機構 FAQ 及守則知識庫...")
def initialize_knowledge_base():
    documents = []
    
    # 策略 A: 載入本地法例與實務指示 (Cap 620, PD 31, PD 31.1)
    local_files = ["data/Cap620.md", "data/PD31.md", "data/PD31_1.md"]
...
system_prompt = (
    ...
    "你必須遵守以下嚴格管治規則：\n"
    "1. 你的回答【必須且只能】基於提供的 Context（香港法例第620章《調解條例》、實務指示 PD 31 / PD 31.1、司法機構調解 FAQ、HKMAAL 守則、HKMC 規則或 HKIAC 調解規則）。\n"
    "2. 如果 Context 中沒有明確答案，你必須回答：『根據現有知識庫，無法提供確切答案，建議諮詢資深調解員或參考律師意見。』絕對不允許憑空捏造法律條文或法律意見。\n"
    "3. 在回答中，必須明確引用資料來源（例如：『根據司法機構調解 FAQ』或『根據《調解條例》第 8 條』）。\n"
...
if user_query := st.chat_input("請輸入關於香港調解程序、司法機構指引或保密條例的問題..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("檢索《調解條例》、實務指示及司法機構 FAQ 中..."):
            try:
